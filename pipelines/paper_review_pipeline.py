...
from ..minerU.minerU import MinerUClient
...
class PaperReviewPipeline:
    ...
    def __init__(
        self,
        config: Optional[PaperReviewConfig] = None,
        **kwargs
    ):
        """
    ...
    def _init_mineru(self):
        ...
        try:
            self.mineru_client = MinerUClient(
                api_key=self.config.mineru.api_key
            )
    ...
    def _init_llm(self):
        ...
        try:
            ...
            logger.info(f"LLM client initialized with model: {self.config.llm.model_name}")
        except Exception as e:
    ...
    def _prepare_review_prompt(
        ...
        """Prepare the review prompt for the LLM."""
        ...
        conference_info = self._get_conference_info(conference)
        
        prompt = f"""Please review the following research paper for {conference_info['name']} conference.

Conference Standards:
- Acceptance Rate: {conference_info['acceptance_rate']:.1%}
- Minimum Confidence Score: {conference_info['min_confidence']}/10

Review Criteria:
{chr(10).join(f"- {criterion}" for criterion in self.config.llm.review_criteria)}
...
Format your response as a structured review."""
    ...
    def _parse_review_response(
        ...
        """Parse the LLM review response into structured format."""
        try:
            ...
            if "accept" in response.lower():
                ...
                assessment = "Reject"
            elif "revision" in response.lower():
                assessment = "Revision"
            
            # Extract confidence score
            import re
            confidence_match = re.search(r'confidence.*?(\d+)/10', response.lower())
            confidence_score = int(confidence_match.group(1)) if confidence_match else 5
            
            # Conference fit assessment
            conference_info = self._get_conference_info(conference)
            is_competitive = confidence_score >= conference_info["min_confidence"]
            
            return {
                "assessment": assessment,
                "confidence_score": confidence_score,
                "is_competitive": is_competitive,
                "conference_standards": conference_info,
            ...
            return {
                ...
                "conference_standards": self._get_conference_info(conference),
                "full_response": response,
                "parsing_error": str(e)
            }
    ...
    def _save_results(self, result: Dict[str, Any], input_path: str):
        ...
        try:
            ...
            output_file = Path(self.config.mineru.output_dir) / f"{input_name}_review.{self.config.output_format}"
    ...
    def _save_as_markdown(self, result: Dict[str, Any], output_file: Path):
        ...
        with open(output_file, 'w', encoding='utf-8') as f:
            ...
            f.write(f"- **Assessment:** {review['assessment']}\n")
            f.write(f"- **Confidence Score:** {review['confidence_score']}/10\n")
            f.write(f"- **Competitive for Conference:** {review['is_competitive']}\n\n")
            
            f.write(f"## Full Review\n\n")
            f.write(f"{result['review']['full_response']}\n\n")
            
            f.write(f"## Parsed Content\n\n")
            f.write(f"*Content length: {result['parsed_content']['file_size']} characters*\n\n")
